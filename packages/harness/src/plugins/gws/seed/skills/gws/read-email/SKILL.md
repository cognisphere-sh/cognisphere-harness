...
messages your gws settings filtered out (e.g. `Cc`/`Bcc`-only mail when
`requireAgentInTo` is on — the thread listing is the only place you'll see them).
...
You are only woken for the newest message of a thread the poller delivers —
and your `scripts/gws/settings` decide which those are. Anything else — an
older message, filtered-out mail, your own sent mail — you have to go and find:
