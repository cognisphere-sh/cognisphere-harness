...
- Changes to harness data (agent.json, plugin config, prompts) take effect on the next agent restart — tell the operator when a restart (`cognisphere dev`/`serve` locally, `sudo ./scripts/server.sh restart` on the box) is needed.
